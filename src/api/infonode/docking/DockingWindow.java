...
import api.infonode.docking.FocusManager;
import api.infonode.docking.drag.DockingWindowDragger;
import api.infonode.docking.drop.ChildDropInfo;
import api.infonode.docking.drop.DropFilter;
import api.infonode.docking.drop.SplitDropInfo;
import api.infonode.docking.internal.ReadContext;
import api.infonode.docking.internal.WindowAncestors;
import api.infonode.docking.internal.WriteContext;
import api.infonode.docking.internalutil.DropAction;
import api.infonode.docking.location.LocationDecoder;
import api.infonode.docking.model.SplitWindowItem;
import api.infonode.docking.model.TabWindowItem;
import api.infonode.docking.model.ViewWriter;
import api.infonode.docking.model.WindowItem;
import api.infonode.docking.properties.DockingWindowProperties;
import api.infonode.docking.title.DockingWindowTitleProvider;
import api.infonode.docking.title.SimpleDockingWindowTitleProvider;
import api.infonode.docking.util.DockingUtil;
import api.infonode.gui.ComponentUtil;
import api.infonode.gui.EventUtil;
import api.infonode.gui.mouse.MouseButtonListener;
import api.infonode.gui.panel.BasePanel;
import api.infonode.properties.propertymap.*;
...
/**
 * This is the base class for all types of docking windows. The windows are structured in a tree,
 * typically with a {@link RootWindow} at the root. Each DockingWindow has a window parent and a
 * number of child windows.
 * <p>
 * <b>Warning: </b> the non-public methods in this class can be changed in non-compatible ways in
 * future versions.
 *
...
abstract public class DockingWindow extends BasePanel {
	...
	private final PropertyMapListener propertiesListener = (PropertyMap propertyMap, Map changes) -> {
		doUpdate();
		updateButtonVisibility();
	};

	private final PropertyMapTreeListener propertyObjectTreeListener = (Map changes) -> {
		doUpdate();
	};
	...
	/**
	 * <p>
	 * Sets the preferred minimize direction of this window. If the {@link WindowBar} in this
	 * direction is enabled this window will be placed on that bar when {@link #minimize()} is
	 * called.
	 * </p>
	 *
	 * <p>
	 * Note that a window will "remember" the last {@link WindowBar} it was added to so the
	 * preferred minimize direction is changed when the window is added to another
	 * {@link WindowBar}.
	 * </p>
	 *
	 * @param direction the preferred minimize direction of this window, null (which is default
	 * value) means use the closest, enabled {@link WindowBar}
	 * @since IDW 1.3.0
	 */
	 ...
	 * {@link #setPreferredMinimizeDirection(api.infonode.util.Direction)} for more information.
	 * </p>
	 *
	 * @return the preferred minimize direction of this window, null if the closest
	 * {@link WindowBar} is used
	 * @since IDW 1.3.0
	 */
	...
	/**
	 * <p>
	 * Adds a listener that receives mouse button events for window tabs. The listener will be
	 * called when a mouse button is pressed, clicked or released on a window tab of this window or
	 * a descendant of this window.
	 * </p>
	 *
	 * <p>
	 * The listeners are called in the reverse order they were added, so the last added listener
	 * will be called first. When all the listeners of this window has been called, the event is
	 * propagated up to the window parent of this window, if there is one.
	 * </p>
	 *
	 * <p>
	 * The {@link MouseEvent}source is the docking window connected to the tab in which the mouse
	 * event occured. The event point is the mouse coordinate where the event occured relative to
	 * the window.
	 * </p>
	 *
	...
	public void removeTabMouseButtonListener(MouseButtonListener listenerDocking) {
		if (getMouseButtonListeners() != null) {
			if (getMouseButtonListeners().remove(listenerDocking) && getMouseButtonListeners().isEmpty()) {
	...
	/**
	 * Splits this window in the given direction. If this window is a View which is contained in a
	 * TabWindow with a single tab, the TabWindow will splitted instead of this View.
	 *
	...
	/**
	 * Same as {@link #restore()}, but the
	 * {@link DockingWindowListener#windowRestoring(DockingWindow)} method of the window listeners
	 * will be called before restoring the window, giving them the possibility to abort the restore
	 * operation.
	 *
	...
	/**
	 * Restores this window to the location before it was minimized, maximized or closed. If the
	 * window can't be restored to the exact same location, a good approximation is performed. It's
	 * not guaranteed that the window is shown anywhere after this method has returned.
	 */
	@SuppressWarnings("unchecked")
	public void restore() {
		if (isMaximized()) {
...
//    	DockingWindow lastFocused = getLastFocusedChildWindow();
			...
			for (int i = 0; i < views.size(); i++) {
				...
				for (DockingWindow a1 : a) {
					a1.doFireWindowRestored(window);
				}
			}
...
//			if (getRootWindow() != null && lastFocused != null)
	...
	/**
	 * <p>
	 * Removes this window from it's window parent. If the window parent is a split window or a tab
	 * window with one child, it will be removed as well.</p>
	 *
	 * <p>
	 * The location of this window is saved and the window can be restored to that location using
	 * the {@link #restore()} method.</p>
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowClosed(DockingWindow)} method of
	 * all the listeners of this window and all window ancestors. The listeners of child windows
	 * will not be notified, for example closing a tab window containing views will not notify the
	 * listeners of views in that tab window.</p>
	 */
	public void close() {
		if (windowParent != null) {
			DockingWindow[] ancestors = getAncestors();
			optimizeAfter(windowParent, () -> {
	...
	/**
	 * Same as {@link #close()}, but the {@link DockingWindowListener#windowClosing(DockingWindow)}
	 * method of the window listeners will be called before closing the window, giving them the
	 * possibility to abort the close operation.
	 *
	...
	/**
	 * <p>
	 * Undocks this window from it's window parent i.e. creates a {@link FloatingWindow} containing
	 * this window.</p>
	 *
	 * <p>
	 * The window can be docked again by calling {@link #dock()}.</p>
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowUndocked(DockingWindow)} method
	 * of all the listeners of this window and all window ancestors. The listeners of child windows
	 * will not be notified, for example undocking a tab window containing views will not notify the
	 * listeners of views in that tab window.</p>
	 *
	...
	/**
	 * Same as {@link #undock(Point)}, but the
	 * {@link DockingWindowListener#windowUndocking(DockingWindow)} method of the window listeners
	 * will be called before undocking the window, giving them the possibility to abort the undock
	 * operation.
	 *
	 * @param location floating window location in screen coordinates
	 * @return the floating window containing the undocked window
	...
	/**
	 * <p>
	 * Docks the window to the RootWindow to the location it had before it was undocked.</p>
	 *
	 * <p>
	 * If the window can't be docked to the exact same location, a good approximation is performed.
	 * It's not guaranteed that the window is shown anywhere after this method has returned.
	 * </p>
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowDocked(DockingWindow)} method of
	 * all the listeners of this window and all window ancestors. The listeners of child windows
	 * will not be notified, for example docking a tab window containing views will not notify the
	 * listeners of views in that tab window.</p>
	 *
	 * @since IDW 1.4.0
	 */
	...
	/**
	 * Same as {@link #dock()}, but the {@link DockingWindowListener#windowDocking(DockingWindow)}
	 * method of the window listeners will be called before docking the window, giving them the
	 * possibility to abort the dock operation.
	 *
	...
	/**
	 * Returns the popup menu factory for this window. If it's null the window parent popup menu
	 * factory will be used when the mouse popup trigger is activated on this window.
	 *
	...
	/**
	 * Sets the popup menu factory for this window. If it's not null a popup menu will be created
	 * and shown when the mouse popup trigger is activated on this window.
	 *
	 ...
	 *
	 * @return the child window that last contained focus, null if no child window has contained
	 * focus or the child has been removed from this window
	 */
	...
	/**
	 * Maximizes this window in its root window or in its floating window. If this window has no
	 * root window nothing happens. This method takes the window component and displays it at the
	 * top in the root window or in the floating window. It does NOT modify the window tree
	 * structure, ie the window parent remains the unchanged.
	 *
	 * <p>
	 * The location of this window is saved and the window can be restored to that location using
	 * the {@link #restore()} method.</p>
	...
	/**
	 * Same as {@link #maximize()}, but the
	 * {@link DockingWindowListener#windowMaximized(DockingWindow)} method of the window listeners
	 * will be called before maximizing the window, giving them the possibility to abort the
	 * maximize operation.
	 *
	...
	/**
	 * Returns true if this window has a root window and is maximized in that root window or in a
	 * floating window.
	 *
	 * @return true if this window has a root window and is maximized in that root window or in a
	 * floating window
	 * @since IDW 1.1.0
	 */
	...
	/**
	 * Minimizes this window. The window is minimized to the {@link WindowBar} in the preferred
	 * minimize direction, see {@link #setPreferredMinimizeDirection(api.infonode.util.Direction)}
	 * and {@link #getPreferredMinimizeDirection()}. If the {@link WindowBar} in that direction is
	 * not enabled, or the direction is null, thiw window is placed on the closest enabled
	 * {@link WindowBar}. If no suitable {@link WindowBar} was found or this window already is
	 * minimized, no action is performed.
	 *
	 * <p>
	 * The location of this window is saved and the window can be restored to that location using
	 * the {@link #restore()} method.</p>
	...
	/**
	 * Minimizes this window to a {@link WindowBar}located in <tt>direction</tt>. If no suitable
	 * {@link WindowBar}was found or this window already is minimized, no action is performed.
	 *
	 * <p>
	 * The location of this window is saved and the window can be restored to that location using
	 * the {@link #restore()} method.</p>
	...
	/**
	 * Same as {@link #minimize()}, but the
	 * {@link DockingWindowListener#windowMinimizing(DockingWindow)} method of the window listeners
	 * will be called before minimizing the window, giving them the possibility to abort the
	 * minimize operation.
	 *
	...
	/**
	 * Same as {@link #minimize(Direction)}, but the
	 * {@link DockingWindowListener#windowMinimizing(DockingWindow)} method of the window listeners
	 * will be called before minimizing the window, giving them the possibility to abort the
	 * minimize operation.
	 *
	...
	/**
	 * Makes this window visible. This causes the tabs of all {@link TabWindow} parents containing
	 * this window to be selected.
	 *
	 * @since IDW 1.1.0
	 */
	...
	/**
	 * Requests that the last focused child window becomes visible and that focus is restored to the
	 * last focused component in that window. If no child window has had focus or the child window
	 * has been removed from this window, focus is transferred to a child component of this window.
	 *
	 * @since IDW 1.1.0
	 */
	...
	private void fireWindowRemoved(DockingWindow removedFromWindow, DockingWindow removedWindow) {
		if (getListeners() != null) {
			...
			for (DockingWindowListener l1 : l) {
				l1.windowRemoved(removedFromWindow, removedWindow);
			}
		}
	...
	protected void fireViewFocusChanged(View previouslyFocusedView, View focusedView) {
		if (getListeners() != null) {
			...
			for (DockingWindowListener l1 : l) {
				l1.viewFocusChanged(previouslyFocusedView, focusedView);
			}
		}
	}
	...
	private void fireWindowAdded(DockingWindow addedToWindow, DockingWindow addedWindow) {
		if (getListeners() != null) {
			...
			for (DockingWindowListener l1 : l) {
				l1.windowAdded(addedToWindow, addedWindow);
			}
		}
	...
	void fireWindowDocked(ArrayList dockedViews) {
		for (int i = 0; i < dockedViews.size(); i++) {
			...
			for (DockingWindow ancestor : ancestors) {
				ancestor.doFireWindowDocked(window);
			}
		}
	}
						...
						if (w != null) {
							...
							optimizeAfter(w.getWindowParent(), () -> {
								if (fitem.getParent() instanceof SplitWindowItem) {
									...
									DockingWindow w2 = getContainer(splitWindowItem, windowItem);
									DockingWindow w3 = w1.getContainer(splitWindowItem, w1.windowItem);
									newWindow.setWindows(isLeft ? w2 : w3, isLeft ? w3 : w2);
