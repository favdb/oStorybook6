...
/**
 * PanelColor: Clever Color Chooser with "no color" and color palette (both optional).
 *
 * @author martin
 *
 */
...
public class ColorChooserPanel extends JPanel implements IRefreshable, MouseListener {
	...
	public ColorChooserPanel(String title, Color startColor, Color[] colors, boolean allowNoColor) {
		this.title = title;
	...
	private void initUi() {
		MigLayout layout = new MigLayout(MIG.INS0);
		setLayout(layout);
		// shows the selected color
		lbShowColor = new ColorLabel(startColor);
		// custom palette
		if (colors != null) {
			btPalette = new IconButton(BT_PALETTE, ICONS.K.PALETTE, getShowPalettePopupAction());
			btPalette.set20x20();
		...
		if (allowNoColor) {
			btClearColor = new IconButton("btClearColor", ICONS.K.DELETE, getClearColorAction());
			btClearColor.set20x20();
		}
		add(lbShowColor);
		if (colors != null) {
			add(btPalette);
	...
	/**
	 * Returns its self for use within anonymous objects that require references to this object without being able to
	 * use <code>this</code> keyword.
	 *
	 * @return
	 */
