 ...
 * @author favdb
 */
public class ScenarioEdit extends AbstractPanel {
	...
	public ScenarioEdit(TypistScenario scenarioPanel) {
	...
	@SuppressWarnings("OverridableMethodCallInConstructor")
	public ScenarioEdit(SceneEdit scenePanel) {
	...
	public void initUi() {
		...
		tfPitch.setFont(App.getInstance().fonts.editorGet());
		...
		add(cbMoment);
		// radio des INT/EXT
		add(new JSLabel(I18N.getColonMsg("location")));
		...
		add(new JSLabel(I18N.getColonMsg("scenario.end")),
	...
	public JComboBox initCbScenario(String name) {
		...
		cb.setName(name);
		for (int i = 0;; i++) {
			String s = Scenes.scenarioKey(name, i);
			if (s.isEmpty()) {
				break;
			}
			cb.addItem(s);
