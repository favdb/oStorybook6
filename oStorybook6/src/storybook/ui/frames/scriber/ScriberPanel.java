 ...
 */
package storybook.ui.frames.scriber;
...
import storybook.ui.panels.typist.TypistEntitiesListDlg;
import storybook.ui.panels.typist.TypistInfo;
import storybook.ui.panels.typist.TypistScenario;

/**
 * JPanel to edit a Scene
 *
 * @author favdb
 */
public class ScriberPanel extends AbstractPanel {

	private static final String TT = "Scriber.";
	...
	private JPanel initMiddle() {
		...
		infoPanel = new TypistInfo(this, null);
	...
	public JToolBar initToolbar() {
		...
		toolbar.add(initTbMain(), MIG.GROWX);
		return toolbar;
	}
	...
	private JPanel initTbMain() {
		...
		tbMain = new JPanel(new MigLayout(MIG.get(MIG.INS0, MIG.GAP1)));
		tbMain.setOpaque(false);
		tbMain.add(initTB2());

		tbMain.add(new JLabel(" "), MIG.GROWX);

		// ignore changes
		tbMain.add(btIgnore = Ui.initButton(BT_IGNORE, "", ICONS.K.CANCEL,
				"discard.changes", e -> refresh()));
		// save changes
		tbMain.add(btSave = Ui.initButton(BT_SAVE, "", ICONS.K.F_SAVE,
				"file.save", e -> save()));
		return tbMain;
	}

	/**
	 * initialize second toolbar (title, chapter, status, intensity, strand)
	 *
	 * @return
	 */
	@SuppressWarnings("unchecked")
	private JPanel initTB2() {
		...
		JPanel tb2 = new JPanel(new MigLayout(MIG.get(MIG.get(MIG.FILLX, MIG.INS0))));
		tb2.setOpaque(false);
		//title of the Scène, and new Scene
		tb2.add(tfTitle = Ui.initStringField(tb2, "tfTitle", 32, "", ""));
		tb2.add(btNewScene = Ui.initButton(BT_NEW, "", ICONS.K.ADD,
				"scene.new", e -> sceneNew()));
		tb2.add(new JLabel(" "));
		//panel for main infos (chapter, status, intensity, strand)
		JPanel tb21 = new JPanel(new MigLayout(MIG.INS0));
		//chapitre
		tb21.add(initLabel(CHAPTER, true));
		...
		tb21.add(initLabel("status", true));
		tb21.add(cbStatus);
		//intensity
		intensityPanel = new IntensityPanel(scene.getIntensity().get());
		tb21.add(initLabel("intensity", true));
		tb21.add(intensityPanel);
		//strand
		tb21.add(new JLabel(""), MIG.GROWX);
		tb21.add(initLabel("strands", true));
		...
		// empty separator
		tb2.add(new JSeparator(), MIG.GROWX);

		// panel for ckReview and btShowInfo
		JPanel tb22 = new JPanel(new MigLayout(MIG.INS0));
	...
	/**
	 * initialize the Review panel, not usable for Markdown
	 *
	 * @return
	 */
