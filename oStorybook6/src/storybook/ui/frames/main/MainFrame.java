 ...
 */
package storybook.ui.frames.main;
...
import storybook.ui.panels.BlankPanel;
import storybook.ui.panels.StatusbarPanel;
...
public class MainFrame extends JFrame implements IPaintable {
	...
	public boolean isTypist = false, isEpisode = false, isScenario = false, isScriber = false;
	...
	/**
	 * MainFrame class with only Font initialization
	 *
	 */
	public MainFrame() {
	...
	/**
	 * MainFrame class with Project initialization
	 *
	 * @param dbF
	 */
	...
	public void initUi() {
		...
		// set title, dimension, close operation
		setTitle();
		restoreDimension();
	...
	public void scriberSet() {
		isScriber = true;
	}

	public void scriberUnset() {
		isScriber = false;
	}
	...
	public void episodeActivate() {
		//LOG.trace(TT+"activateTipist()=>" + (isTypist ? "true" : "false"));
		if (isEpisode) {
			isEpisode = false;
	...
	/**
	 * initialize the full frame for Typist mode
	 */
	...
	/**
	 * initialize the Infonode rootwindow
	 *
	 */
	private void initRootWindow() {
		//LOG.trace(TT+"initRootWindow()");
	...
	public void setDefaultLayout() {
		...
		// intialize the part containing Tree view and Info view
	...
	/**
	 * initialize after pack
	 */
	private void initAfterPack() {
		//LOG.trace(TT+"initAfterPack()");
		SbView scenesView = getView(VIEWNAME.SCENES);
		SbView locationsView = getView(VIEWNAME.LOCATIONS);
		SbView personsView = getView(VIEWNAME.PERSONS);
		SbView bookView = getView(VIEWNAME.BOOK);
		...
		// load initially shown views
		SbView[] views2 = {scenesView,
			personsView, locationsView,
			bookView, treeView, quickInfoView};
	...
	/**
	 * get the SbView by VIEWNAME
	 *
	 * @param view
	 * @return
	 */
	public SbView getView(VIEWNAME view) {
		return viewFactory.getView(view);
	}

	/**
	 * show the given VIEWNAME
	 *
	 * @param viewName
	 */
	public void showView(VIEWNAME viewName) {
		//LOG.trace(TT+"showView(" + viewName.name() + ")");
		cursorSetWaiting();
	...
	/**
	 * show and focus the given VIEWNAME
	 *
	 * @param viewName
	 */
	public void showAndFocus(VIEWNAME viewName) {
		//LOG.trace(TT+"showAndFocus(" + viewName.name() + ")");
		System.gc();
		SbView view = getView(viewName);
		view.restore();
		view.restoreFocus();
	...
	/**
	 * close the given VIEWNAME
	 *
	 * @param viewName
	 */
	public void closeView(VIEWNAME viewName) {
		//LOG.trace(TT + "closeView(" + viewName.name() + ")");
		SbView view = getView(viewName);
		view.close();
	}
	...
	public void close(boolean forceExit) {
		//LOG.trace(TT + "close(forceExit=" + (forceExit ? "true" : "false") + ")");
	...
	/**
	 * copy an iamge from ressource to the give file
	 *
	 * @param src
	 * @return
	 */
	public String imageCopy(String src) {
		//LOG.trace(TT + "imageCopy(src='" + src + "')");
		if (src.startsWith("file://")) {
			File f = new File(src.replace("file://", ""));
			File imgF = new File(project.getImageDir() + File.separator + f.getName());
			if (!imgF.exists()) {
	...
	public void changePath(String oldPath, String newPath) {
		//LOG.trace(TT+"changePath(" + oldPath + ", " + newPath + ")");
	...
	/**
	 * save the Project to an other file
	 */
	...
	/**
	 * rename the file Project
	 */
	public void fileRename() {
		//LOG.trace(TT+"fileRename()");
	...
	public int askForSave() {
		//LOG.trace(TT + "askForSave() modified=" + (isUpdated() ? "true" : "false"));
		if (isUpdated()) {
			if (isTypist) {
				if (isScenario) {
	...
	public void updateLanguage() {
		for (int i = 0; i < viewFactory.getViewMap().getViewCount(); ++i) {
			...
			if (SbViewFactory.isTable(view) && view.isLoaded()) {
	...
	private class MainFrameWindowAdaptor extends WindowAdapter {
		...
		public void windowClosing(WindowEvent evt) {
			//LOG.trace("MainFrameWindowAdaptor.windowClosing(evt=" + evt.toString() + ")");
			if (askForSave() != 0) {
				modified = false;
				close(true);
			}
	...
	private class MainDockingWindowAdapter extends DockingWindowAdapter {
		...
		public void windowAdded(DockingWindow addedToWindow, DockingWindow addedWindow) {
			//LOG.trace("MainDockingWindowAdapter.windowAdded("
			//		+ addedToWindow.getName() + ", " + addedWindow.getName() + ")");
		...
		public void windowClosed(DockingWindow window) {
			//LOG.trace("MainDockingWindowAdapter.windowClosed("
			//		+ window.getName() + ")");
	...
	/**
	 * save the layout of the current window
	 */
	...
	public String getClipboardContents() {
		...
		try {
			...
			LOG.err(TT + "getClipboardContents() error", ex);
		}
		return result;
	}
