...
public class TreeTransferHandler
	...
	public boolean executeDrop(TreeEntity targetTree, TreeNode dragged, TreeNode target, int action) {
		if (action == DnDConstants.ACTION_MOVE) {
			...
			if (draggedObj.equals(targetObj)) {
				return false;
			}
	...
	private void dropSceneBeforeScene(Scene scene, Scene target) {
		...
		Chapter newChapter = target.getChapter();
		List<Scene> sc = (newChapter == null ? mainFrame.project.scenes.findUnassigned()
				: mainFrame.project.scenes.find(newChapter));
		if (Objects.equals(oldChapter, newChapter)) {
			sc.remove(scene);
		}
		scene.setChapter(newChapter);
		...
		mainFrame.project.scenes.renumber(newChapter);
