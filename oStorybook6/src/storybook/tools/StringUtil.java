...
public class StringUtil {
	...
	public static String javaDecode(String s) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (c == '\\') {
				if (i + 1 >= s.length()) {
					LOG.err("javaDecode(s" + s + ") case '\\\\' format exception i=" + i);
					return "";
				}
				c = s.charAt(++i);
				switch (c) {
					...
					case 'u': {
						if (i + 4 >= length) {
							LOG.err("javaDecode(s" + s + ") case 'u' format exception i=" + i);
							return "";
						}
						try {
							c = (char) Integer.parseInt(s.substring(i + 1, i + 5), 16);
						} catch (NumberFormatException e) {
							LOG.err("javaDecode(s" + s + ") case 'u' number format exception i=" + i);
							return "";
						}
						i += 4;
						buff.append(c);
					...
					default:
						if (c >= '0' && c <= '9' && i + 2 < length) {
							try {
								...
								LOG.err("javaDecode(s" + s + ") case default number format exception i=" + i);
								return "";
							}
							i += 2;
							buff.append(c);
