...
public class SpellUtil {
	...
	public static File getDictionaryDir() throws IOException {
		try {
			...
			File file = new File(
				...
				+ File.separator
				+ Const.SpellCheker.DICTIONARIES);
			if (!file.exists()) {
				file.mkdir();
			}
			return file;
	...
	public static void registerDictionaries() {
		try {
			...
			String spelling = App.preferences.getString(Pref.KEY.SPELLING, "none");
			if (!spelling.equals("none")) {
				...
				LOG.trace("Initialize Dictionary from " + uDictDir.toString());
			}
		} catch (IOException e) {
		}
	}
	...
	public static List<Language> getLanguages() {
		...
		InputStream stream = App.getInstance().getClass().getResourceAsStream("/resources/languages/languages.txt");
		if (stream == null) {
			LOG.err("unable to open the languages.txt file");
			return languages;
		}
		InputStreamReader istr = new InputStreamReader(stream);
		BufferedReader txt = new BufferedReader(istr);
		String line = null;
		try {
			line = txt.readLine();
		} catch (IOException ex) {
			LOG.err("unable to read languages.txt", ex);
		}
		while (line != null) {
			String[] s = line.split(",");
			if (s.length == 2) {
				languages.add(new Language(s[1], s[0]));
			}
			try {
				line = txt.readLine();
			} catch (IOException ex) {
