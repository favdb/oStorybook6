...
public class Editor extends AbstractPanel {
	...
	private JButton btEntityPrior, btEntityNext;
	...
	@Override
	public void init() {
		//LOG.trace(TT + "init() entitytype=" + objType);
		withPart = false;
	}
	...
	/**
	 * set the next entity for navigation
	 */
	private void setEntityNext() {
		//LOG.trace(TT + "setEntityNext()");
	...
	/**
	 * set the prior entity for navigation
	 */
	private void setEntityPrior() {
		//LOG.trace(TT + "setEntityPrior()");
		AbstractEntity e = EntityUtil.getPrior(mainFrame, entity);
		if (e != null) {
			setEntity(e);
			ctrl.setTableRow(e);
	...
	/**
	 * initialize the panel for the current entity
	 */
	...
	/**
	 * the the icon for the current entity
	 *
	 * @param str
	 */
