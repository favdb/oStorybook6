...
abstract class DictionaryBase {
	...
	/**
	 * It will search with different rules for similar words. For every rule there are a different
	 * difference. The smaller the difference that so similarer. This method is calling recursive it
	 * self.
	 *
	 * @param list Container for found words
	 * @param chars until charPosition already map characters, after the part of the original word
	 * that still need to map.
	 ...
	 */
	private void searchSuggestions(Suggestions list, CharSequence chars,
			int charPosition, int lastIdx, int diff) {
		...
		if (charPosition < chars.length()) {
			...
			while (idx < size && tree[idx] < LAST_CHAR) {
				if (isWordMatch()) {
					...
					list.add(new Suggestion(buffer,
							diff + 5 + (chars.length() - buffer.length()) * 5));
				}
				if (charPosition + 1 < chars.length()) {
					char newChar = tree[idx];
					idx = readIndex();
					if (idx > 0 && newChar != currentChar) {
						...
						searchSuggestions(list, buffer,
								charPosition + 1, idx, diff + charDiff(currentChar, newChar));
					}
				}
				idx = tempIdx += 3;
			}
	...
	private void searchSuggestionsLonger(Suggestions list,
			CharSequence chars, int lastIdx, int diff) {
		int tempIdx = idx = lastIdx;
		while (idx < size && tree[idx] < LAST_CHAR) {
			StringBuilder buffer = new StringBuilder();
			buffer.append(chars);
	...
	/**
	 * Search if the character exist in the current node. If found then the variable
	 * <code>idx</code> point to the location. If not found then it point on the next character
	 * (char value) item in the node.
	 *
	...
	/**
	 * Returns an int that describe the dissimilarity of the characters. The value is ever larger 0.
	 * A value of means only a small difference.
	 *
	 * @param a first char
	 * @param b second char
