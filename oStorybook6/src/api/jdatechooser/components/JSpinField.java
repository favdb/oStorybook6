...
/**
 * JSpinField is a numeric field with 2 spin buttons to increase or decrease the value. It has the
 * same interface as the "old" JSpinField but uses a JSpinner internally (since J2SE SDK 1.4) rather
 * than a scrollbar for emulating the spin buttons.
 *
 * @author Kai Toedter
...
public class JSpinField extends JPanel implements ChangeListener, CaretListener, ActionListener,
   FocusListener {
	...
	/**
	 * Default JSpinField constructor. The valid value range is between Integer.MIN_VALUE and
	 * Integer.MAX_VALUE. The initial value is 0.
	 */
	public JSpinField() {
	...
	/**
	 * After any user input, the value of the textfield is proofed. Depending on being an integer,
	...
	/**
	 * After any user input, the value of the textfield is proofed. Depending on being an integer,
	 * the value is colored green or red. If the textfield is green, the enter key is accepted and
