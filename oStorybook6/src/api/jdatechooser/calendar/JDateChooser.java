...
/**
 * A date chooser containing a date editor and a button, that makes a JCalendar visible for choosing
 * a date. If no date editor is specified, a JTextFieldDateEditor is used as default.
 *
 * @author Kai Toedter
...
public class JDateChooser extends JPanel implements ActionListener,
	 ...
	 *
	 * @param date the date or null
	 * @param dateFormatString the date format string or null (then MEDIUM SimpleDateFormat format
	 * is used)
	 */
	...
	/**
	 * Creates a new JDateChooser. If the JDateChooser is created with this constructor, the mask
	 * will be always visible in the date editor. Please note that the date pattern and the mask
	 * will not be changed if the locale of the JDateChooser is changed.
	 *
	...
	public JDateChooser(JCalendar jcal, Date date, String dateFormatString,
		...
		calendarButton = new JButton(IconUtil.getIconSmall(ICONS.K.CALENDAR)) {
	...
	/**
	 * Returns the date. If the JDateChooser is started with a null date and no date was set by the
	...
	/**
	 * Sets a valid date range for selectable dates. If max is before min, the default range with no
	 * limitation is set.
	 *
	...
	/**
	 * Should only be invoked if the JDateChooser is not used anymore. Due to popup handling it had
	 * to register a change listener to the default menu selection manager which will be
	 * unregistered here. Use this method to cleanup possible memory leaks.
	 */
	public void cleanup() {
