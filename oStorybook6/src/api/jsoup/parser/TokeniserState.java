...
enum TokeniserState {
	...
	BogusComment {
		void read(Tokeniser t, CharacterReader r) {
			// t o d o : handle bogus comment starting from eof. when does that trigger?
			// rewind to capture character that lead us here
			r.unconsume();
			t.commentPending.append(r.consumeTo('>'));
			// t o d o : replace nullChar with replaceChar
			char next = r.consume();
			if (next == '>' || next == eof) {
	...
	MarkupDeclarationOpen {
		void read(Tokeniser t, CharacterReader r) {
			if (r.matchConsume("--")) {
				...
				// t o d o : should actually check current namepspace, and only non-html allows cdata. until namespace
