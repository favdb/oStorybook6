...
public class WysiwygEditorKit extends HTMLEditorKit {
	...
	public void deinstall(JEditorPane ed) {
		...
		if (curAct instanceof HTMLEditPasteAction) {
	...
	/**
	 * Fetch a factory that is suitable for producing views of any models that are produced by this
	 * kit.
	 *
	 * @return the factory
	 */
	...
	@SuppressWarnings("unchecked")
	private void addAction(Map actions, ActionMap actionMap, String lib, Action action) {
		Action delegate = actionMap.get(lib);
		actions.put(lib, delegate);
		actionMap.put(lib, action);
	}

	/**
	 * Factory to build views of the html elements.
	 *
	 * This simply extends the behavior of the default html factory to draw borderless tables, etc
	 */
	...
	private class ResizeHandler extends MouseInputAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			...
			switch (dragDir) {
				case ResizableView.SE:
					r.width = e.getX() - r.x;
					r.height = e.getY() - r.y;
					break;
				case ResizableView.NE:
					r.width = e.getX() - r.x;
					r.height = (r.y + r.height) - e.getY();
					r.y = e.getY();
					break;
				case ResizableView.SW:
					r.width = (r.x + r.width) - e.getX();
					r.height = e.getY() - r.y;
					r.x = e.getX();
					break;
				case ResizableView.NW:
					r.width = (r.x + r.width) - e.getX();
					r.height = (r.y + r.height) - e.getY();
					r.x = e.getX();
					r.y = e.getY();
					break;
				case ResizableView.N:
					r.height = (r.y + r.height) - e.getY();
					r.y = e.getY();
					break;
				case ResizableView.S:
					r.height = e.getY() - r.y;
					break;
				case ResizableView.E:
					r.width = e.getX() - r.x;
					break;
				case ResizableView.W:
					r.width = (r.x + r.width) - e.getX();
					r.x = e.getX();
					break;
				default:
		...
		/**
		 * Updates the list of monitored ResizeableViews. If they don't exist in the document,
		...
		private void updateMonitoredViews(HTMLDocument doc) {
			for (Iterator it = monitoredViews.iterator(); it.hasNext();) {
				...
				if (vElem.getName().equals("img")) {
					...
					//get the parent and check if it's the same element
	 ...
	 * <p>
	 * The default view for such {@link Element}s are big ugly blocky JTextFields, which doesn't
	 * really work well in practice and tends to confuse users. This view replaces the default and
	 * simply draws the elements as blocks that indicates the presence of an uneditable
	 * {@link Element}.
	 * <p>
	 *
	 * @author Bob Tantlinger
