...
import api.infonode.gui.draggable.DraggableComponentBox;
import api.infonode.tabbedpanel.Tab;
import api.infonode.tabbedpanel.TabbedPanel;
import api.infonode.tabbedpanel.TabbedPanelContentPanel;
      ...
      if (tp.isTabAreaVisible()) {
        ...
        if (tp.getTabCount() > 0) {
          ...
          if (PAINT_TAB_AREA && !(pane.getTabCount() == 0 && tabData.getTabCount() > 0)) {
            ...
            if (tabData.isHorizontalLayout()) {
              int extraWidth = lastTab.getWidth() == lastVisibleRect.width ? 0 : 2 * tabData.getTabbedPanelSize().width
