...
import api.infonode.docking.action.*;
import api.infonode.docking.drop.ChildDropInfo;
import api.infonode.docking.drop.InteriorDropInfo;
import api.infonode.docking.internal.HeavyWeightContainer;
import api.infonode.docking.internal.HeavyWeightDragRectangle;
...
import api.infonode.docking.model.*;
import api.infonode.docking.properties.RootWindowProperties;
import api.infonode.docking.util.DockingUtil;
import api.infonode.gui.CursorManager;
import api.infonode.gui.DragLabelWindow;
import api.infonode.gui.componentpainter.ComponentPainter;
import api.infonode.gui.componentpainter.RectangleComponentPainter;
import api.infonode.gui.layout.BorderLayout2;
import api.infonode.gui.layout.LayoutUtil;
import api.infonode.gui.layout.StretchLayout;
import api.infonode.gui.mouse.MouseButtonListener;
import api.infonode.gui.panel.SimplePanel;
import api.infonode.gui.shaped.panel.ShapedPanel;
import api.infonode.properties.gui.InternalPropertiesUtil;
import api.infonode.properties.propertymap.PropertyMap;
import api.infonode.properties.propertymap.PropertyMapManager;
import api.infonode.util.ArrayUtil;
import api.infonode.util.Direction;
import api.infonode.util.ReadWritable;

/**
 * The root window is a top level container for docking windows. Docking windows can't be dragged
 * outside of their root window. The property values of a root window is inherited to the docking
 * windows inside it.
 *
...
public class RootWindow extends DockingWindow implements ReadWritable {
	...
	/**
	 * Creates a root window with support for heavyweight components inside the views and the given
	 * window inside as window inside this root window.
	 *
	 * @param heavyweightSupport true for heavy weight component support, otherwise false
	 * @param viewSerializer used when reading and writing views
	 * @param window the window that is placed inside the root window
	...
	/**
	 * Returns the property values for this root window. The property values will be inherited to
	 * docking windows inside this root window.
	 *
	...
	/**
	 * Returns the direction of the closest enabled window bar to a docking window. The distance is
	 * measured from the window edge that is furthest away from the bar.
	 *
	...
	public Direction getClosestWindowBar(DockingWindow window) {
		Point pos = SwingUtilities.convertPoint(window.getParent(),
				window.getLocation(), this);
		...
		Direction dir
				= new Direction[]{Direction.UP,
					Direction.DOWN,
					Direction.LEFT,
					Direction.RIGHT}[ArrayUtil.findSmallest(distances)];
	...
	/**
	 * <p>
	 * Creates and shows a floating window with the given window as top-level window in the floating
	 * window or without any top-level window i.e. empty floating window.
	 * </p>
	 *
	 * <p>
	 * <strong>Note 1:</strong> The created floating window is not visible per default. To make it
	 * visible, call {@link FloatingWindow}.getTopLevelAncestor().setVisible(true);
	 * </p>
	 *
	 * <p>
	 * <strong>Note 2:</strong> Floating windows are dynamically created when a window is undocked
	 * and closed/removed when all windows inside the floating window has been removed (i.e.
	 * cloased/docked/undocked to another floating window) from the floating window. The root window
	 * has a refernce to the floating window as long as the floating window has windows inside it
	 * i.e. it is not necessary to keep references to the floating window because the root window
	 * will handle this.
	 * </p>
	 *
	 * @param location the floating window's location on the screen
	 * @param innerSize the inner dimension of the floating window's top level container i.e.the
	 * size of the root pane
	 * @param window the docking window that is the top level window in this floating window or null
	 * for no top-level window i.e. empty floating window
	 ...
	 *
	 * @param out the stream on which to write the state
	 * @param writeProperties true if the property values for all docking windows should be written
	 * to the stream
	 * @throws IOException if there is a stream error
	 */
	@SuppressWarnings("unchecked")
	...
	private void writeWindowItemIndex(WindowItem item,
	...
	private void writeMaximized(DockingWindow window,
			ObjectOutputStream out) throws IOException {
	...
	private static void writeViews(ArrayList views,
	...
	private void newInternalRead(ObjectInputStream in, ReadContext context) throws IOException {
		...
		try {
			...
			ViewReader viewReader = new ViewReader() {
				@Override
				public ViewItem readViewItem(ObjectInputStream in,
						ReadContext context) throws IOException {
					View view = readView(in, context);
					return view == null ? new ViewItem() : (ViewItem) view.getWindowItem();
				}

				@Override
				public WindowItem readWindowItem(ObjectInputStream in,
						ReadContext context) throws IOException {
					if (in.readBoolean()) {
						int index;
				...
				@Override
				public TabWindow createTabWindow(DockingWindow[] childWindows,
	 ...
	 *
	 * @param in the stream from which to read the state
	 * @param readProperties true if the property values for all child windows should be read. This
	 * parameter can be set to true or false regardless of if the property values was included when
	 * the state was written, though obviously no property values are read if there aren't any in
	 * the stream.
	 * @throws IOException if there is a stream error
	 */
		...
		try {
			...
			if (serializeVersion > SERIALIZE_VERSION) {
				throw new IOException(
						"Can't read serialized data "
						+ "because it was written by a later version"
						+ " of InfoNode Docking Windows!");
			}
	...
	/**
	 * Sets the maximized window in this root window. This method takes the window component and
	 * displays it at the top in the root window. It does NOT modify the window tree structure, ie
	 * the window parent remains the unchanged.
	 *
	...
	/**
	 * Removes all internal references to a view. It's not possible to restore the view to the
	 * previous location after this method has been called.
	 *
	...
	private void createWindowBars() {
		...
		for (int i = 0; i < directions.length; i++) {
			...
			windowBars[i].addPropertyChangeListener("enabled", (PropertyChangeEvent evt) -> {
	...
	protected void update() {
		...
		if (!heavyweightSupport) {
			ShapedPanel dragRect = (ShapedPanel) this.dragRectangle;
			InternalPropertiesUtil.applyTo(properties.getDragRectangleShapedPanelProperties(), dragRect);

			if (dragRect.getComponentPainter() == null) {
				dragRect.setComponentPainter(
	...
	void setDragText(Point textPoint, String text) {
		if (textPoint != null) {
			JRootPane curDragRootPane = getCurrentDragRootPane();

			if (reparent(curDragRootPane.getLayeredPane(), dragTextContainer)) {
				curDragRootPane.getLayeredPane().setLayer(dragTextContainer,
			...
			Point p2 = SwingUtilities.convertPoint(curDragRootPane, textPoint, dragTextContainer.getParent());
			...
			// Drag text window
			Point rp = SwingUtilities.convertPoint(curDragRootPane, textPoint, getRootPane());
			...
			if (!getRootPane().contains(rp) && !floatingWindowsContainPoint(rp)) {
				...
				if (dragTextWindow != null) {
					...
					SwingUtilities.convertPointToScreen(textPoint, curDragRootPane);
	...
	boolean windowBarEnabled() {
		for (WindowBar windowBar : windowBars) {
			if (windowBar.isEnabled()) {
				return true;
			}
		}
